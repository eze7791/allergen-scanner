...
import { Scan, Search, Utensils } from "lucide-react"

const navItems: { label: string; to: string; icon: LucideIcon }[] = [
  { label: "Scan", to: "/", icon: Scan },
    ...
    <>
      ...
      <aside className="hidden md:flex fixed inset-y-0 left-0 z-30 w-56 flex-col border-r border-sidebar-border bg-sidebar">
        <div className="flex h-14 items-center gap-2 border-b border-sidebar-border px-5">
          ...
          <span className="text-sm font-semibold text-sidebar-foreground">Allergen Scan</span>
        </div>
        ...
        <div className="border-t border-sidebar-border p-3">
          <div className="rounded-lg bg-sidebar-accent/50 px-3 py-2">
            <p className="text-[11px] text-sidebar-foreground/60 leading-tight">
              Take a photo of ingredients to detect allergens
            </p>
          </div>
        </div>
      </aside>
